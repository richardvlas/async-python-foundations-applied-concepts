...
class Chat:
    ...
    async def start_db(self):
        # await connection to our redis instance
        self.redis = await aioredis.create_redis_pool("redis://localhost")
        # we are setting the room name to what we received when we instantiated
        # the class
    ...
    async def save_message(self, message_dictionary):
        ...
        # Push the string message_json into redis list
    ...
    async def get_all_messages(self):
        room_name = await self.redis.get("room_name")
        # message_jsons is everything that is in the list named after the 
        # romm_name and that we access through lrange
